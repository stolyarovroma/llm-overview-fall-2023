...
Для этого нам нужны два параметра - scale (Δ) для масштаба и zero-point (z) для сдвига. Тогда квантизованное значение $x_Q$  получается следующим образом:

$x_{int} = round(\dfrac{x}{Δ}) + z$

$x_Q = clamp(0, N_{levels} - 1, x_{int}), (то есть ограничиваем снизу 0, а сверхуN_{levels} - 1)$
...
$x_Q = clamp(0, N_{levels} - 1,  x_{int})$, иначе
